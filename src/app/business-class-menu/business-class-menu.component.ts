...
export class BusinessClassMenuComponent {
  ...
  filteredMenu$ = combineLatest([
    this.menu$,
    this.searchFormControl.valueChanges.pipe(startWith(null)),
  ]).pipe(map(([menu, searchTerm]) => this.filterMenu(menu, searchTerm)));
